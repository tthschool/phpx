<?php
    function get_user($conn  ,$id= null, $name = null)
    {
        $db = new Takesql($conn);
        $users = new User;
    ...
    // include("model.php");
    //login
    function login($username,$pass, $user_list)
    {
        session_start();
    ...
    // sign up and create admin -- not yet
    ...
    function signup($name , $username , $password ,$confirm_password , $user_list,  $is_admin ,$conn)
    {
        ...
        //check if username already exist then
        //need to call get user function
        foreach($user_list as $user){
